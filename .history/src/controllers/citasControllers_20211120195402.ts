...
const reagendarCita = async (req, res) => {
    const { cta_fecharegistro, cta_estado, mot_id, con_id, cta_fechaInicio, cta_fechaFin, per_id } = req.body;
    try {
        const response = await executeQuery(`UPDATE tbl_citas SET cta_fecharegistro = '${cta_fecharegistro}', cta_estado = '${cta_estado}', tbl_motivosconsulta_mot_id = '${mot_id}', tbl_consultorio_con_id = '${con_id}', cta_fechaInicio = '${cta_fechaInicio}', cta_fechaFin = '${cta_fechaFin}', tbl_paciente_per_id= '${per_id}' WHERE cta_id = ${req.params.cta_id}`);
        //res.send('reagendarCita desde el controlador')
